...
clear all
clc
syms Source % S is the source value which can be inputted by the user. Taken as 35.
theta=0.5;
time=0; dt=0.2;
...
while Diff>Converge
    ...
    for i=1:nSrc
        Rside(src_num(i,1))=Rside(src_num(i,1))+Source;
    end
    % Monitorng the temperature at Boundary Nodes.
    Temp_at_node=zeros(nT1,1);
    for i=1:nT1
        Loc=bdy_num(i);
        Temp_at_node(i)=Rside(Loc);
    end
    Temp_at_node
    Tnew=Lhs\Rside;
...
fid = fopen('HW6Temperatures-d.txt','w');
fprintf(fid,'  These are the Temperature values at the boundary nodes \n');
fprintf(fid,'Temperature   Bdy node num \n');
for i=1:nT1
    fprintf(fid,'   %d      %8.4f   \n',Temp_at_node(i),bdy_num(i));
end
%% List of functions.
