...
for i = 1:nSrc
    ...
    nn_min=1;
    n=nSrc;
    src_num=randi([nn_min nn_max],nSrc,1); %% Generating random nodes at which the source term can be applied to.
end    
fclose(fid)
...
while Diff>Converge
    ...
    for i=1:nSrc
        Rside(src_num(i))=Rside(src_num(i))+Source;
    end
...
fid = fopen('HW6Temperatures-e.txt','w');
